void lowLow(int pool_size = 10,
              double low_Pt_threshold = 1)
{
    ...
    TH2F *delta_hist = new TH2F("delta_hist", "Low-Low Two Particle Correlation",
    ...
    for(Int_t nEv = 0; nEv < nEvents; nEv++) // loop over all events
    {
        ...
        for(Int_t nTr = 0; nTr < nTracks; nTr++) // loop over tracks
        {
            ...
            if (current_particle_Pt < low_Pt_threshold)
            {
